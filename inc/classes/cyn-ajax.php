...
if (!class_exists('cyn_ajax')) {
    ...
    class cyn_ajax
    {
        ...
        function __construct()
        {
            ...
            add_action('wp_ajax_cyn_course_form', [$this, 'cyn_course_form']);
            add_action('wp_ajax_nopriv_cyn_course_form', [$this, 'cyn_course_form']);
        }
        ...
        public function cyn_course_form()
        {
            if (!wp_verify_nonce($_POST['_nonce'], 'ajax-nonce'))
                return wp_send_json_error(['verify_nonce' => false], 403);
            ...
            $dbData = array(
                ...
                'user_last_name' => sanitize_email($data['last-name']),
                'user_phone' => sanitize_text_field($data['phone']),
                'user_age' => sanitize_text_field($data['age']),
            ...
            $msg_content = "
                ...
                شماره: " . $dbData['user_phone'] . " \n
                سن : " . $dbData['user_age'] . " \n
                کلاس مورد نظر: " . '<a href="' . get_permalink($dbData['class']) . '"> ' . get_the_title($dbData['class']) . '</a>'  . "
                
            ";

            $new_post = array(
                'post_type' => $GLOBALS["course_submit_form-post-type"],
                'post_title' => $dbData['user_name'],
                'post_content' => $msg_content,
                'post_status' => 'publish',
                'post_author' => 1,
