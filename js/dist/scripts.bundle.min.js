(() => {
  ...
  var _lazyRender = function _lazyRender2() {
    var l = _lazyTweens.length, a = _lazyTweens.slice(0), i, tween2;
    _lazyLookup = {};
    _lazyTweens.length = 0;
    for (i = 0; i < l; i++) {
      tween2 = a[i];
      tween2 && tween2._lazy && (tween2.render(tween2._lazy[0], tween2._lazy[1], true)._lazy = 0);
    }
  };
  ...
  var _rewindStartAt = function _rewindStartAt2(tween2, totalTime, suppressEvents, force) {
    return tween2._startAt && (_reverting ? tween2._startAt.revert(_revertConfigNoKill) : tween2.vars.immediateRender && !tween2.vars.autoRevert || tween2._startAt.render(totalTime, true, force));
  };
  ...
  var _attemptInitTween = function _attemptInitTween2(tween2, time, force, suppressEvents, tTime) {
    _initTween(tween2, time, tTime);
    if (!tween2._initted) {
      return 1;
    }
    if (!force && tween2._pt && !_reverting && (tween2._dur && tween2.vars.lazy !== false || !tween2._dur && tween2.vars.lazy) && _lastRenderedFrame !== _ticker.frame) {
      _lazyTweens.push(tween2);
      tween2._lazy = [tTime, suppressEvents];
      return 1;
    }
  };
  ...
  var _renderZeroDurationTween = function _renderZeroDurationTween2(tween2, totalTime, suppressEvents, force) {
    var prevRatio = tween2.ratio, ratio = totalTime < 0 || !totalTime && (!tween2._start && _parentPlayheadIsBeforeStart(tween2) && !(!tween2._initted && _isFromOrFromStart(tween2)) || (tween2._ts < 0 || tween2._dp._ts < 0) && !_isFromOrFromStart(tween2)) ? 0 : 1, repeatDelay = tween2._rDelay, tTime = 0, pt, iteration, prevIteration;
    if (repeatDelay && tween2._repeat) {
      tTime = _clamp(0, tween2._tDur, totalTime);
      iteration = _animationCycle(tTime, repeatDelay);
      tween2._yoyo && iteration & 1 && (ratio = 1 - ratio);
      if (iteration !== _animationCycle(tween2._tTime, repeatDelay)) {
        prevRatio = 1 - ratio;
        tween2.vars.repeatRefresh && tween2._initted && tween2.invalidate();
      }
    }
    if (ratio !== prevRatio || _reverting || force || tween2._zTime === _tinyNum || !totalTime && tween2._zTime) {
      if (!tween2._initted && _attemptInitTween(tween2, totalTime, force, suppressEvents, tTime)) {
        return;
      }
      prevIteration = tween2._zTime;
      tween2._zTime = totalTime || (suppressEvents ? _tinyNum : 0);
      suppressEvents || (suppressEvents = totalTime && !prevIteration);
      tween2.ratio = ratio;
      tween2._from && (ratio = 1 - ratio);
      tween2._time = 0;
      tween2._tTime = tTime;
      pt = tween2._pt;
      while (pt) {
      ...
      totalTime < 0 && _rewindStartAt(tween2, totalTime, suppressEvents, true);
      tween2._onUpdate && !suppressEvents && _callback(tween2, "onUpdate");
      tTime && tween2._repeat && !suppressEvents && tween2.parent && _callback(tween2, "onRepeat");
      if ((totalTime >= tween2._tDur || totalTime < 0) && tween2.ratio === ratio) {
        ratio && _removeFromParent(tween2, 1);
        if (!suppressEvents && !_reverting) {
          _callback(tween2, ratio ? "onComplete" : "onReverseComplete", true);
          tween2._prom && tween2._prom();
        }
      }
    } else if (!tween2._zTime) {
      tween2._zTime = totalTime;
    }
  };
  ...
  var Timeline = /* @__PURE__ */ function(_Animation) {
    ...
    _proto2.tweenTo = function tweenTo(position, vars) {
      vars = vars || {};
      var tl = this, endTime = _parsePosition(tl, position), _vars = vars, startAt = _vars.startAt, _onStart = _vars.onStart, onStartParams = _vars.onStartParams, immediateRender = _vars.immediateRender, initted, tween2 = Tween.to(tl, _setDefaults({
        ease: vars.ease || "none",
        lazy: false,
        ...
        onStart: function onStart() {
          ...
          if (!initted) {
            ...
            tween2._dur !== duration && _setDuration(tween2, duration, 0, 1).render(tween2._time, true, true);
            initted = 1;
          }
          _onStart && _onStart.apply(tween2, onStartParams || []);
        }
      }, vars));
      return immediateRender ? tween2.render(0) : tween2;
    };
  ...
  var _processVars = function _processVars2(vars, index, target, targets, tween2) {
    _isFunction(vars) && (vars = _parseFuncOrString(vars, tween2, index, target, targets));
    if (!_isObject(vars) || vars.style && vars.nodeType || _isArray(vars) || _isTypedArray(vars)) {
      return _isString(vars) ? _parseFuncOrString(vars, tween2, index, target, targets) : vars;
    }
    var copy = {}, p;
    for (p in vars) {
      copy[p] = _parseFuncOrString(vars[p], tween2, index, target, targets);
    }
    return copy;
  };
  var _checkPlugin = function _checkPlugin2(property, vars, tween2, index, target, targets) {
    var plugin, pt, ptLookup, i;
    if (_plugins[property] && (plugin = new _plugins[property]()).init(target, plugin.rawVars ? vars[property] : _processVars(vars[property], index, target, targets, tween2), tween2, index, targets) !== false) {
      tween2._pt = pt = new PropTween(tween2._pt, target, property, 0, 1, plugin.render, plugin, 0, plugin.priority);
      if (tween2 !== _quickTween) {
        ptLookup = tween2._ptLookup[tween2._targets.indexOf(target)];
  ...
  var _initTween = function _initTween2(tween2, time, tTime) {
    var vars = tween2.vars, ease = vars.ease, startAt = vars.startAt, immediateRender = vars.immediateRender, lazy = vars.lazy, onUpdate = vars.onUpdate, onUpdateParams = vars.onUpdateParams, callbackScope = vars.callbackScope, runBackwards = vars.runBackwards, yoyoEase = vars.yoyoEase, keyframes = vars.keyframes, autoRevert = vars.autoRevert, dur = tween2._dur, prevStartAt = tween2._startAt, targets = tween2._targets, parent = tween2.parent, fullTargets = parent && parent.data === "nested" ? parent.vars.targets : targets, autoOverwrite = tween2._overwrite === "auto" && !_suppressOverwrites, tl = tween2.timeline, cleanVars, i, p, pt, target, hasPriority, gsData, harness, plugin, ptLookup, index, harnessVars, overwritten;
    tl && (!keyframes || !ease) && (ease = "none");
    tween2._ease = _parseEase(ease, _defaults.ease);
    tween2._yEase = yoyoEase ? _invertEase(_parseEase(yoyoEase === true ? ease : yoyoEase, _defaults.ease)) : 0;
    if (yoyoEase && tween2._yoyo && !tween2._repeat) {
      yoyoEase = tween2._yEase;
      tween2._yEase = tween2._ease;
      tween2._ease = yoyoEase;
    }
    tween2._from = !tl && !!vars.runBackwards;
    if (!tl || keyframes && !vars.stagger) {
      ...
      if (startAt) {
        _removeFromParent(tween2._startAt = Tween.set(targets, _setDefaults({
          data: "isStart",
          overwrite: false,
          parent,
        ...
        tween2._startAt._dp = 0;
        tween2._startAt._sat = tween2;
        time < 0 && (_reverting || !immediateRender && !autoRevert) && tween2._startAt.revert(_revertConfigNoKill);
        if (immediateRender) {
          if (dur && time <= 0 && tTime <= 0) {
            time && (tween2._zTime = time);
            return;
          }
        }
        ...
        if (!prevStartAt) {
          ...
          _removeFromParent(tween2._startAt = Tween.set(targets, p));
          tween2._startAt._dp = 0;
          tween2._startAt._sat = tween2;
          time < 0 && (_reverting ? tween2._startAt.revert(_revertConfigNoKill) : tween2._startAt.render(-1, true));
          tween2._zTime = time;
          if (!immediateRender) {
            _initTween2(tween2._startAt, _tinyNum, _tinyNum);
          } else if (!time) {
            return;
          }
        }
      }
      tween2._pt = tween2._ptCache = 0;
      lazy = dur && _isNotFalse(lazy) || lazy && !dur;
      for (i = 0; i < targets.length; i++) {
        target = targets[i];
        gsData = target._gsap || _harness(targets)[i]._gsap;
        tween2._ptLookup[i] = ptLookup = {};
        _lazyLookup[gsData.id] && _lazyTweens.length && _lazyRender();
        index = fullTargets === targets ? i : fullTargets.indexOf(target);
        if (harness && (plugin = new harness()).init(target, harnessVars || cleanVars, tween2, index, fullTargets) !== false) {
          tween2._pt = pt = new PropTween(tween2._pt, target, plugin.name, 0, 1, plugin.render, plugin, 0, plugin.priority);
        ...
        if (!harness || harnessVars) {
          for (p in cleanVars) {
            if (_plugins[p] && (plugin = _checkPlugin(p, cleanVars, tween2, index, target, fullTargets))) {
              plugin.priority && (hasPriority = 1);
            } else {
              ptLookup[p] = pt = _addPropTween.call(tween2, target, p, "get", cleanVars[p], index, fullTargets, 0, vars.stringFilter);
            }
          }
        }
        tween2._op && tween2._op[i] && tween2.kill(target, tween2._op[i]);
        if (autoOverwrite && tween2._pt) {
          _overwritingTween = tween2;
          _globalTimeline.killTweensOf(target, ptLookup, tween2.globalTime(time));
          overwritten = !tween2.parent;
          _overwritingTween = 0;
        }
        tween2._pt && lazy && (_lazyLookup[gsData.id] = 1);
      }
      hasPriority && _sortPropTweensByPriority(tween2);
      tween2._onInit && tween2._onInit(tween2);
    }
    tween2._onUpdate = onUpdate;
    tween2._initted = (!tween2._op || tween2._pt) && !overwritten;
    keyframes && time <= 0 && tl.render(_bigNum, true, true);
  };
  var _updatePropTweens = function _updatePropTweens2(tween2, property, value, start, startIsRelative, ratio, time) {
    var ptCache = (tween2._pt && tween2._ptCache || (tween2._ptCache = {}))[property], pt, rootPT, lookup, i;
    if (!ptCache) {
      ptCache = tween2._ptCache[property] = [];
      lookup = tween2._ptLookup;
      i = tween2._targets.length;
      while (i--) {
        ...
        if (!pt) {
          ...
          tween2.vars[property] = "+=0";
          _initTween(tween2, time);
  ...
  var _parseFuncOrString = function _parseFuncOrString2(value, tween2, i, target, targets) {
    return _isFunction(value) ? value.call(tween2, i, target, targets) : _isString(value) && ~value.indexOf("random(") ? _replaceRandom(value) : value;
  };
  ...
  var _addPluginModifier = function _addPluginModifier2(modifier, tween2, target, property) {
    var pt = this._pt, next;
    while (pt) {
      next = pt._next;
      pt.p === property && pt.modifier(modifier, tween2, target);
      pt = next;
    }
  };
  ...
  var PropTween = /* @__PURE__ */ function() {
    ...
    _proto4.modifier = function modifier(func, tween2, target) {
    ...
    _proto5.kill = function kill2(revert, matchMedia2) {
      ...
      if (revert) {
        ...
        this.data.forEach(function(t) {
          if (t.data === "isFlip") {
            t.revert();
            t.getChildren(true, true, false).forEach(function(tween2) {
              return tweens.splice(tweens.indexOf(tween2), 1);
            });
          }
        });
  ...
  var _gsap = {
    ...
    quickTo: function quickTo(target, property, vars) {
      var _merge22;
      var tween2 = gsap.to(target, _merge((_merge22 = {}, _merge22[property] = "+=0.1", _merge22.paused = true, _merge22), vars || {})), func = function func2(value, start, startIsRelative) {
        return tween2.resetTo(property, value, start, startIsRelative);
      };
      func.tween = tween2;
      return func;
    },
  ...
  var _addModifiers = function _addModifiers2(tween2, modifiers) {
    var targets = tween2._targets, p, i, pt;
    for (p in modifiers) {
      i = targets.length;
      while (i--) {
        pt = tween2._ptLookup[i][p];
        if (pt && (pt = pt.d)) {
          if (pt._pt) {
            pt = _getPluginPropTween(pt, p);
          }
          pt && pt.modifier && pt.modifier(modifiers[p], tween2, targets[i], p);
        }
      }
    }
  };
  var _buildModifierPlugin = function _buildModifierPlugin2(name, modifier) {
    return {
      ...
      init: function init5(target, vars, tween2) {
        tween2._onInit = function(tween3) {
          var temp, p;
          if (_isString(vars)) {
            temp = {};
  ...
  var gsap = _gsap.registerPlugin({
    name: "attr",
    init: function init(target, vars, tween2, index, targets) {
      var p, pt, v;
      this.tween = tween2;
      for (p in vars) {
  ...
  var _specialProps = {
    clearProps: function clearProps(plugin, target, property, endValue, tween2) {
      if (tween2.data !== "isFromStart") {
  ...
  _forEachName("padding,margin,Width,Radius", function(name, index) {
    ...
    _specialProps[index > 1 ? "border" + name : name] = function(plugin, target, property, endValue, tween2) {
      var a, vars;
      ...
      plugin.init(target, vars, tween2);
    };
  });
  var CSSPlugin = {
    name: "css",
    ...
    init: function init3(target, vars, tween2, index, targets) {
      var props = this._props, style = target.style, startAt = tween2.vars.startAt, startValue, endValue, endNum, startNum, type, specialProp, p, startUnit, endUnit, relative, isTransformRelated, transformPropTween, cache, smooth, hasPriority, inlineProps;
      _pluginInitted || _initCore();
      ...
      for (p in vars) {
        ...
        endValue = vars[p];
        if (_plugins[p] && _checkPlugin(p, vars, tween2, index, target, targets)) {
          continue;
        }
        type = typeof endValue;
        specialProp = _specialProps[p];
        if (type === "function") {
          endValue = endValue.call(tween2, index, target, targets);
          type = typeof endValue;
        }
        ...
        if (specialProp) {
          specialProp(this, target, p, endValue, tween2) && (hasPriority = 1);
          ...
          if (startAt && p in startAt) {
            startValue = typeof startAt[p] === "function" ? startAt[p].call(tween2, index, target, targets) : startAt[p];
  ...
  var _getBounds = function _getBounds2(element, withoutTransforms) {
    var tween2 = withoutTransforms && _getComputedStyle(element)[_transformProp2] !== "matrix(1, 0, 0, 1, 0, 0)" && gsap3.to(element, {
      x: 0,
      y: 0,
      xPercent: 0,
      yPercent: 0,
  ...
  var _getTweenCreator = function _getTweenCreator2(scroller, direction) {
    var getScroll = _getScrollFunc(scroller, direction), prop = "_scroll" + direction.p2, getTween = function getTween2(scrollTo, vars, initialValue, change1, change2) {
      var tween2 = getTween2.tween, onComplete = vars.onComplete, modifiers = {};
      ...
      modifiers[prop] = function() {
        return checkForInterruption(initialValue + change1 * tween2.ratio + change2 * tween2.ratio * tween2.ratio);
      };
      ...
      vars.onComplete = function() {
        getTween2.tween = 0;
        onComplete && onComplete.call(tween2);
      };
      tween2 = getTween2.tween = gsap3.to(scroller, vars);
      return tween2;
    };
    ...
    _proto.init = function init5(vars, animation) {
      ...
      if (snap3) {
        ...
        snapDelayedCall = gsap3.delayedCall(snap3.delay || scrubSmooth / 2 || 0.1, function() {
          var scroll = scrollFunc(), refreshedRecently = _getTime2() - lastRefresh < 500, tween2 = tweenTo.tween;
          if ((refreshedRecently || Math.abs(self.getVelocity()) < 10) && !tween2 && !_pointerIsDown && lastSnap !== scroll) {
            ...
            if (scroll <= end && scroll >= start && endScroll !== scroll) {
              if (tween2 && !tween2._initted && tween2.data <= _abs(endScroll - scroll)) {
                return;
              }
  ...
  var _getScrollNormalizer = function _getScrollNormalizer2(vars) {
    ...
    var _vars2 = vars, normalizeScrollX = _vars2.normalizeScrollX, momentum = _vars2.momentum, allowNestedScroll = _vars2.allowNestedScroll, onRelease = _vars2.onRelease, self, maxY, target = _getTarget(vars.target) || _docEl2, smoother = gsap3.core.globals().ScrollSmoother, smootherInstance = smoother && smoother.get(), content = _fixIOSBug && (vars.content && _getTarget(vars.content) || smootherInstance && vars.content !== false && !smootherInstance.smooth() && smootherInstance.content()), scrollFuncY = _getScrollFunc(target, _vertical), scrollFuncX = _getScrollFunc(target, _horizontal), scale = 1, initialScale = (Observer.isTouch && _win4.visualViewport ? _win4.visualViewport.scale * _win4.visualViewport.width : _win4.outerWidth) / _win4.innerWidth, wheelRefresh = 0, resolveMomentumDuration = _isFunction3(momentum) ? function() {
      ...
      skipTouchMove = true;
    }, tween2, startScrollX, startScrollY, onStopDelayedCall, onResize2 = function onResize3() {
      updateClamps();
      if (tween2.isActive() && tween2.vars.scrollY > maxY) {
        scrollFuncY() > maxY ? tween2.progress(1) && scrollFuncY(maxY) : tween2.resetTo("scrollY", maxY);
      }
    };
    ...
    vars.onRelease = vars.onGestureStart = function(self2, wasDragging) {
      ...
      if (!wasDragging) {
        ...
        if (normalizeScrollX) {
          ...
          tween2.vars.scrollX = scrollClampX(endScroll);
        }
        currentScroll = scrollFuncY();
        endScroll = currentScroll + dur * 0.05 * -self2.velocityY / 0.227;
        dur *= _clampScrollAndGetDurationMultiplier(scrollFuncY, currentScroll, endScroll, _maxScroll(target, _vertical));
        tween2.vars.scrollY = scrollClampY(endScroll);
        tween2.invalidate().duration(dur).play(0.01);
        if (_fixIOSBug && tween2.vars.scrollY >= maxY || currentScroll >= maxY - 1) {
          gsap3.to({}, {
            onUpdate: onResize2,
            duration: dur
          });
  ...
  var ScrollToPlugin = {
    ...
    init: function init4(target, value, tween2, index, targets) {
    ...
    render: function render3(ratio, data) {
      var pt = data._pt, target = data.target, tween2 = data.tween, autoKill = data.autoKill, xPrev = data.xPrev, yPrev = data.yPrev, isWin = data.isWin, snap3 = data.snap, snapInline = data.snapInline, x, y, yDif, xDif, threshold;
      while (pt) {
        pt.r(ratio, pt.d);
        pt = pt._next;
      }
      ...
      if (autoKill) {
        ...
        if (data.skipX && data.skipY) {
          tween2.kill();
          data.vars.onAutoKill && data.vars.onAutoKill.apply(tween2, data.vars.onAutoKillParams || []);
        }
      }
      if (isWin) {
  ...
  // js/modules/front/front-gsap.js
  gsapWithCSS.registerPlugin(ScrollTrigger2, ScrollToPlugin);
  var sectionContainer = document.querySelector("#panels-container2");
  var sectionTween;
  var sectionPanels = gsapWithCSS.utils.toArray("#panels-container2 .panel2");
  ...
  sectionTween = gsapWithCSS.to(sectionPanels, {
  ...
  // js/modules/front/swiper.js
  var coachSlider = document.querySelector(".coach-slider");
  if (coachSlider) {
  ...
  // js/modules/front/faq.js
  var acardion = document.querySelectorAll(".accardion");
  ...
  // js/modules/front/testimonial.js
  ...
  playIcons.forEach(function(playIcon) {
    playIcon.addEventListener("click", function() {
      ...
      videoPopup.innerHTML = '<video controls autoplay>                <source src="video.mp4" type="video/mp4">                \u0645\u0631\u0648\u0631\u06AF\u0631 \u0634\u0645\u0627 \u0627\u0632 \u0648\u06CC\u062F\u06CC\u0648 \u067E\u0634\u062A\u06CC\u0628\u0627\u0646\u06CC \u0646\u0645\u06CC\u200C\u06A9\u0646\u062F.                </video>                <button class="close-popup">&#10006;</button>';
  ...
  tabCategory.forEach((category) => {
    category.addEventListener("click", (e) => {
      ...
      blogs.forEach((blogs2) => {
        const slug = category.getAttribute("data-slug");
        console.log(slug);
        if (blogs2.classList.contains(slug)) {
          blogs2.classList.add("active-blogs");
          heightFunc(blogs2);
          window.onresize = function(event) {
            heightFunc(blogs2);
          };
        } else {
          blogs2.classList.remove("active-blogs");
        }
      });
